...
def build_bilstm_model(input_shape):
    input_layer = Input(shape=input_shape)
    lstm_out = Bidirectional(LSTM(64, return_sequences=False))(input_layer)  # Return sequences=False for final output
    dense_out = Dense(64, activation='relu')(lstm_out)
    output_layer = Dense(1)(dense_out)
...
def build_gru_model(input_shape):
    input_layer = Input(shape=input_shape)
    lstm_out = GRU(64, return_sequences=False)(input_layer)  # Return sequences=False for final output
    dense_out = Dense(64, activation='relu')(lstm_out)
    output_layer = Dense(1)(dense_out)
...
#     input_layer = Input(shape=input_shape)
#     lstm_out = LSTM(64, return_sequences=True)(input_layer)  # Shape: (batch, timesteps, lstm_units)
#     feature_attention_out = feature_attention_layer(lstm_out)  # Shape: (batch, timesteps, lstm_units)
#     dense_out = TimeDistributed(Dense(1))(feature_attention_out)  # Shape: (batch, timesteps, 1)
...
def build_lstm_attention_model(input_shape):
    ...
    lstm_out = LSTM(64, return_sequences=True)(feature_attention_out)  # Shape: (batch, timesteps, lstm_units)
